            ...
            <div class="row">
              <div class="col-md-12 col-sm-12  ">
                <div class="x_panel" style="border-radius:10px;border-width:2px;">
                  <div class="x_title">
                    <h2>Manage Pet Articles Post</h2>
                                      ...
                                      <div class="card-body" style="box-shadow: 8px 8px 8px #888888;border-radius:10px;">
                                           ...
                                           <i class="fa fa-ellipsis-h menu" style="float: right;" type="button" id="dropdownMenuButton" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false"><i hidden><?php echo ($result->userID);?></i>
                                            ...
                                            <div class="dropdown-menu" aria-labelledby="dropdownMenuButton">
                                              <button hidden class="dropdown-item Epost" data-post-id="<?php echo ($result->postID);?>" data-post-title="<?php echo ($result->postTitle);?>" data-post-content="<?php echo ($result->postContent);?>" data-post-location="<?php echo ($result->postLocation);?>" data-post-time="<?php echo ($result->postTime);?>" data-post-date="<?php echo ($result->postEventdate);?>"><i hidden><?php echo ($result->userID);?></i> Edit</button>

                                              <button class="dropdown-item Dpost" data-post-id="<?php echo ($result->postID);?>" data-master-id="<?php echo ($result->userID);?>"><i hidden><?php echo ($result->userID);?></i> Delete</button>
                                              <button hidden class="dropdown-item Ppost" data-post-id="<?php echo ($result->postID);?>" data-post-picture="<?php echo ($result->postPicture);?>"><i hidden><?php echo ($result->userID);?></i> Change Picture</button>
                                            </div><br>
                                           ...
                                           <h5 class="card-title" id="description" id="Title" name="Title" style="text-align:left;resize:none;width:880px">Title: <?php echo ($result->postTitle);?></h5>
                                              ...
                                              foreach($pictures as $picture)
                                            {
                                              ...
                                              <label style="margin-top:-30px;"><img <?php echo"<img src = '/developgetpet/web/images/$picture->Image'";?> alt="avatar" style="width:30px;height:30px;margin-top:10px;" class="rounded-circle img-responsive">&nbsp<textarea disabled="yes" class="txtgrow" style="width:800px;height:auto;font-size:16px;border-radius:20px; background-color:#e9ecef;resize: none;overflow:hidden;font-size:14px;text-align:left;padding-top: 10px;color: #808080;margin-top:10px;" type='text'><?php echo ( $picture->orgName);?><?php echo ( $picture->userFirstname);?> <?php echo ( $picture->userLastname);?>&#13;&#10;<?php echo ( $comment->commentContent);?></textarea>&nbsp&nbsp<i hidden class="fa fa-ellipsis-v option" type="button" id="dropdownMenuButton" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false"><i hidden><?php echo ($comment->userID);?></i></i>
...
if(isset($_POST['btnComment']))
{
  ...
  if($masterID == $ID)
  {
    ...
    echo "<script type ='text/javascript'> document.location='http://localhost/developgetpet/dashboard/P.O-PostArticles.php'</script>";
...
<?php
   if(isset($_POST['Delete']))
   {
    ...
    $sql3="INSERT INTO notification(activityID,postID,notificationTitle,userID,masterID,notificationDescription,notificationDate,notificationStatus)VALUES(:postID,:postID,'Post Deleted','$ID',:masterID,'Your Post Has Been Removed By Admin','$date','Unread')";
    $query3=$dbh->prepare($sql3);
    $query3->bindParam(':postID',$postID,PDO::PARAM_STR);
    $query3->bindParam(':masterID',$masterID,PDO::PARAM_STR);
    $query3->execute();
    ...
    echo "<script type ='text/javascript'> document.location='http://localhost/developgetpet/dashboard/Admin-Managearticles.php'</script>";
   }
?>
<!-- //Delete Post Code -->
  ...
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      ...
      <form method="post">
        ...
        <div style="text-align: center" class="wrap-input100 validate-input">
					    <input hidden id="master_id1" name="masterID" required = "required" class="form-control" id="success">
				</div>
        <div style="text-align: center" class="form-group">
         <div class="col-md-6 offset-md-3">
...
<script type="text/javascript">
  $(".Dpost").click(function () {
    ...
    var master_id1 = $(this).attr('data-master-id');
    $('#DeletePost').modal('show');
    $("#post_id1").val( post_id1 );
    $("#master_id1").val( master_id1 );
  });
  </script>
